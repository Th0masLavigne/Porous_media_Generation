...
if __name__ == "__main__"  :
	...
	for number_of_seeds in seeds_list:
		...
		Nsph = 1
		# define the cylinder coordinates in mm
		Diameter, Height = 24, 40
		# Define the parameters of the vascular tube
		thickness_tube = 0.3 #[mm]
		rad_i_tube = 0.5
		...
		Diameter_connector_mm = Diameter_connector*25.4 #[mm]
		Diameter_connector_i = Diameter_connector_mm / 2
		Length_connector = 60 #[mm]
		...
		level_fixing = round(thickness_tube/3,2)
		tolerance = 1e-8
		maxiter = 5
		hull = True
		hull_perm = False
		...
		import pymesh
		mesh_1=pymesh.meshio.load_mesh("250_seed_permeabilitty.stl", drop_zero_dim=False)
		print(mesh_1.is_manifold())
		#
		...
		while continue_==True:
		    # if input('Are you satisfied with the current geometry? [y/n]: ') == 'y':
		        ...
		        # 	if iter_>maxiter:
		        # 		break
		        # mesh_perm, mesh_perm_top, mesh_perm_bottom = create_permeabilitty_sample(vertices, edges, radius, Height, rad_i_tube, rad_e_tube, Length_tube, Diameter, Diameter_connector_mm, Diameter_connector_i, Length_connector, Ncyl, Nsph, output_filename_perm)
		        # print(f'mesh is manifold: {mesh_perm.is_manifold()}')
		        ...
		        # 	if iter_>maxiter:
		        # 		break
		        # mesh_perm, mesh_perm_top, mesh_perm_bottom = create_permeabilitty_sample_2(vertices, edges, radius, Height, rad_i_tube, rad_e_tube, Length_tube, Diameter, Diameter_connector_mm, Diameter_connector_i, Length_connector, Ncyl, Nsph, output_filename_perm[:-4])
		        # print(f'mesh is manifold: {mesh_perm.is_manifold()}')
