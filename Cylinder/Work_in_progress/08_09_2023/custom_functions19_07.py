...
def create_permeabilitty_sample(vertices, edges, radius, Height, rad_i_tube, rad_e_tube, Length_tube, Diameter, Diameter_connector_e, Diameter_connector_i, Length_connector, Ncyl, Nsph, output_filename):
	...
	for i in range(len(vertices)):
		if numpy.abs(vertices[i,2] - Height) < 1e-8 or numpy.abs(vertices[i,2]) < 1e-8:
			...
			list_union.append({"mesh": sphere})
	# Insert tube: 
	internal_cylinder  = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,1e-3],
	...
	external_cylinder  = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,1e-3],
	...
	# close the tube
	bottom_cylinder  = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,0],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,1e-3], rad_e_tube, rad_e_tube, num_segments=2*Ncyl)
	top_cylinder  = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height-1e-3],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height], rad_e_tube, rad_e_tube, num_segments=2*Ncyl)
	# Create the chamber
	...
	external_cylinder_2  = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,-chamber_height],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height+chamber_height], Diameter/2+3e-3, Diameter/2+3e-3, num_segments=2*Ncyl)
	# 
	# Create the top and bottom meshes to be glued
	# 
	bottom_chamber  = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,-3e-3-chamber_height],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,-chamber_height], Diameter/2+6e-3, Diameter/2+6e-3, num_segments=2*Ncyl)
	top_chamber  = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height+chamber_height],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height+chamber_height+3e-3], Diameter/2+6e-3, Diameter/2+6e-3, num_segments=2*Ncyl)
	glued_interface_int_top = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height-6e-3],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height+chamber_height+3e-3], Diameter/2+3e-3, Diameter/2+3e-3, num_segments=2*Ncyl)
	glued_interface_ext_top = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height-6e-3],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height+chamber_height+3e-3], Diameter/2+6e-3, Diameter/2+6e-3, num_segments=2*Ncyl)
	glued_interface_int_bottom = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,-chamber_height-3e-3],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,6e-3], Diameter/2+3e-3, Diameter/2+3e-3, num_segments=2*Ncyl)
	glued_interface_ext_bottom = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,-chamber_height-3e-3],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,6e-3], Diameter/2+6e-3, Diameter/2+6e-3, num_segments=2*Ncyl)
...
def create_permeabilitty_sample_ANAS(vertices, edges, radius, Height, rad_i_tube, rad_e_tube, Length_tube, Diameter, Diameter_connector_e, Diameter_connector_i, Length_connector,chamber_height,chamber_width, Ncyl, Nsph, output_filename):
	...
	internal_cylinder_2  = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,-chamber_height],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height+chamber_height], 0.95*Diameter/2, 0.95*Diameter/2, num_segments=2*Ncyl)
	external_cylinder_2  = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,-chamber_height],
	 [(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,Height+chamber_height], Diameter/2+chamber_width, Diameter/2+chamber_width, num_segments=2*Ncyl)
	# 
	# Create the top and bottom meshes to be glued
	# 
	bottom_chamber  = pymesh.generate_cylinder([(numpy.max(vertices[:,0])-numpy.min(vertices[:,0]))/2,(numpy.max(vertices[:,1])-numpy.min(vertices[:,1]))/2,-3e-3-chamber_height],
